...
-(void)encryWithPublicKey
{
    ...
    _encryWithPublicKeyTextview.text = [DES3Util encryptUseDES:_originTexfield.text key:@"helloworld"];
    NSLog(@"%@",_encryWithPublicKeyTextview.text);
    AFHTTPSessionManager * manager = [AFHTTPSessionManager manager];
    
    [manager POST:@"http://192.168.1.198/emoji/testrsa.json" parameters:@{@"result":_encryWithPublicKeyTextview.text,@"key":@"abcdefgh"} constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
...
-(void)decryWithPriviteKey
{
    ...
    _dencryWithPriviteKeyTextview.text = [DES3Util decryptUseDES:@"K/0P1Vy7WONXY5Gx4Pz+Yg==" key:@"helloworld"];
    AFHTTPSessionManager * manager = [AFHTTPSessionManager manager];
    
    [manager POST:@"http://192.168.1.198/emoji/testrsa.json" parameters:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        NSLog(@"%@",[DES3Util decryptUseDES:responseObject[@"result"] key:responseObject[@"key"]]);
        
        _dencryWithPriviteKeyTextview.text =[DES3Util decryptUseDES:responseObject[@"result"] key:responseObject[@"key"]];
